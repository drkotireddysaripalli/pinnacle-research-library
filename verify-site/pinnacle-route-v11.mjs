 ...
 async fetch(request,env,ctx){
  ...
  if(isVerify&&['www.pinnacleblooms.org','pinnacleblooms.org'].includes(incoming.hostname)){
   ...
   if(range&&/^bytes=\d*-\d*(?:,\s*\d*-\d*)*$/.test(range))headers.set('range',range);
    let source;try{
     // Text changes are published from the authenticated Sites origin immediately; binary files keep the
     // Worker Assets fast path. Missing binary assets also fall back to the same reviewed release origin.
     source=textual?await fetch(upstream,{method:request.method,headers,redirect:'manual'}):env.ASSETS?await env.ASSETS.fetch(new Request('https://assets.local'+file,{method:request.method,headers:range?{Range:range}:{}})):await fetch(upstream,{method:request.method,headers,redirect:'manual'});
     if(!textual&&source.status===404)source=await fetch(upstream,{method:request.method,headers,redirect:'manual'});
