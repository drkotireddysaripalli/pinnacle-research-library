...
const PREFIX='/verify';
const RELEASE = "f2e5f5847c3d401b";
...
export const HOME_EVIDENCE_RULE={
 ...
 panel:{compact:true,title:'Explore the evidence behind Pinnacle',
  ...
  primaryLabel:'Explore Pinnacle Verify',primaryUrl:PUBLIC+'/',storyLink:true}
};
...
export function contextualEvidencePanel(rule){
 const p=rule.panel;
 const story=p.storyLink?'<p style="margin:.8rem 0 0"><a href="'+PUBLIC+'/evidence/pinnacleai-regulatory-journey.html" style="color:#056678;font-weight:700;text-decoration:underline;text-underline-offset:.15em">PinnacleAI®: the complete story, research and export readiness</a></p>':'';
 if(p.compact)return '<aside id="'+CONTEXT_MARKER+'" aria-labelledby="pinnacle-evidence-context-title" style="clear:both;box-sizing:border-box;width:100%;max-width:832px;margin:1.5rem auto;padding:1.25rem;border:1px solid #b8dad5;border-radius:12px;background:#fff;color:#143347;text-align:center;line-height:1.6">'
  ...
  +'<a href="'+escapeContext(p.primaryUrl)+'" style="display:inline-block;box-sizing:border-box;max-width:100%;padding:.6rem 1rem;border:1px solid #087f8c;border-radius:6px;color:#056678;font-size:1rem;font-weight:700;text-decoration:underline;text-underline-offset:.15em">'+escapeContext(p.primaryLabel)+'</a>'+story+'</aside>';
 ...
 return '<aside id="'+CONTEXT_MARKER+'" class="pinnacle-evidence-context" aria-labelledby="pinnacle-evidence-context-title" style="margin:1.25rem 0;padding:1.15rem;border:1px solid #b8dad5;border-inline-start:4px solid #087f8c;border-radius:12px;background:#fff;color:#143347;font:inherit;line-height:1.6">'
  ...
  +note+story+'<p style="margin:.8rem 0 0"><a href="'+escapeContext(p.primaryUrl)+'" style="color:#056678;font-weight:700;text-decoration:underline;text-underline-offset:.15em">'+escapeContext(p.primaryLabel)+'</a></p>'
...
export async function transformContextualEvidence(request,response,Rewriter=globalThis.HTMLRewriter,rules=CONTEXTUAL_EVIDENCE_RULES){
 ...
 const existingPanels=[...html.matchAll(/<aside\b[^>]*\bid\s*=\s*(["'])pinnacle-evidence-context\1[^>]*>[\s\S]*?<\/aside>/gi)];
 const hasMarker=/\bid\s*=\s*(["'])pinnacle-evidence-context\1/i.test(html);
 // The main application now owns an evidence panel. Extend only that recognized
 // panel, once, while retaining its native layout and all transformation guards.
 const nativeHome=hasMarker&&rule.id===HOME_EVIDENCE_RULE.id&&existingPanels.length===1
  &&existingPanels[0][0].includes('Explore the evidence behind Pinnacle')
  &&/href=["'](?:https:\/\/www\.pinnacleblooms\.org)?\/verify\/["']/.test(existingPanels[0][0])
  &&!existingPanels[0][0].includes('pinnacleai-regulatory-journey.html');
 if(hasMarker&&!nativeHome)return response;
 const selector=nativeHome?'aside#pinnacle-evidence-context':rule.insertion.selector;
 const method=nativeHome?'append':rule.insertion.method;
 const markup=nativeHome?'<p style="margin:12px auto 20px;text-align:center;font-size:16px;line-height:1.5"><a href="'+PUBLIC+'/evidence/pinnacleai-regulatory-journey.html">PinnacleAI®: the complete story, research and export readiness</a></p>':contextualEvidencePanel(rule);
 ...
 const rewritten=new Rewriter()
  ...
  .on(selector,{element(element){if(inserted||excluded)return;inserted=true;element[method](markup,{html:true});}})
