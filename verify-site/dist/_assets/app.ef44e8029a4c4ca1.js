...
(() => {
  ...
  const revealHash = () => {
    ...
    if(!card.hidden&&!document.querySelector('.reading-jump-bar'))card.scrollIntoView({block:'start'});
  };
  updateCategories(); render(); revealHash(); window.addEventListener('hashchange', revealHash);window.addEventListener('pinnacle:reveal',revealHash);
  ...
  if (centreList && centreData.length) {
    ...
    const revealCentreHash = () => {
      ...
      if(centreMore)centreMore.dataset.expanded='true';centreRender();if(!document.querySelector('.reading-jump-bar'))card.scrollIntoView({block:'start'});
    };
    revealCentreHash();window.addEventListener('hashchange',revealCentreHash);window.addEventListener('pinnacle:reveal',revealCentreHash);
  }
