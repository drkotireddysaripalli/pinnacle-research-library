...
(() => {
 ...
 const languageForm=document.querySelector('[data-translate-form]');
 const readerStatus=document.querySelector('[data-reader-status]');
 const announce=text=>{if(readerStatus)readerStatus.textContent=text;};
 const originalSource=()=>{
  let encoded;try{encoded=decodeURIComponent(languageForm?.dataset.originalUrl||'');}catch{}
 ...
 const navigateLanguage=destination=>{
  ...
  // A direct user-initiated top-level navigation avoids translating an already
  // translated wrapper. It also returns cleanly to our own native guide pages.
  try{window.open(destination,'_top');}catch{location.assign(destination);}
 };
 if(languageForm){
  const select=languageForm.querySelector('select');
  const translatedLanguage=new URL(location.href).searchParams.get('_x_tr_tl')||new URL(location.href).searchParams.get('tl');
  if(select&&[...select.options].some(option=>option.value===translatedLanguage))select.value=translatedLanguage;
  languageForm.addEventListener('submit',event=>{
   event.preventDefault();
   const language=select?.value,destination=languageDestination(originalSource(),language);
 ...
 document.addEventListener('click',event=>{
  ...
  if(target.matches('[data-read-original]')){event.preventDefault();navigateLanguage(originalSource());return;}
  const nativeLanguage=target.dataset.nativeLanguageLink||(target.closest('.guide-lang-nav')?target.hreflang?.split('-')[0]:null);
  if(nativeLanguage){const destination=languageDestination(originalSource(),nativeLanguage);if(destination){event.preventDefault();send('verify_language',{language:nativeLanguage});navigateLanguage(destination);}return;}
  if(target.matches('[data-more-languages]')){event.preventDefault();navigateLanguage(languageDestination(originalSource(),'te'));}
 },true);
 ...
 const applyReadingSize=value=>{
  const size=sizes.includes(value)?value:'1';document.documentElement.style.setProperty('--reader-scale',size);
  document.querySelectorAll('[data-reader-size]').forEach(button=>button.setAttribute('aria-pressed',String(button.dataset.readerSize===size)));
 };
 ...
 document.querySelectorAll('[data-reader-size]').forEach(button=>button.addEventListener('click',()=>{
  const value=button.dataset.readerSize;applyReadingSize(value);try{localStorage.setItem(sizeKey,value);}catch{}
  announce(value==='1'?'Standard text size selected.':value==='1.12'?'Larger text size selected.':'Largest text size selected.');
 }));
