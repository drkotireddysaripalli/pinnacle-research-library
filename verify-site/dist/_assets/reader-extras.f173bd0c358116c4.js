...
(() => {
 ...
 const submitLanguage=form=>{
  const language=form.querySelector('select')?.value,destination=languageDestination(originalSource(form),language);
 ...
 document.addEventListener('click',event=>{
  ...
  if(form&&target.matches('button[type="submit"]')){event.preventDefault();submitLanguage(form);return;}
  if(target.tagName!=='A')return;
  ...
  if(nativeLanguage){const destination=languageDestination(originalSource(target),nativeLanguage);if(destination){event.preventDefault();send('verify_language',{language:nativeLanguage});navigateLanguage(destination);}return;}
  if(target.matches('[data-more-languages]')){event.preventDefault();navigateLanguage(languageDestination(originalSource(target),'te'));}
 },true);
