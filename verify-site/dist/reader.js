...
(()=>{
 ...
 const reveal=(focusTarget=false)=>{
  window.dispatchEvent(new Event('pinnacle:reveal'));
  const node=hashNode();if(!node){if(!location.hash){current=groups[0];readAll=false;showGroups();}return;}
  ...
  if(focusTarget){const focus=node.matches('details')?node.querySelector(':scope>summary'):node.querySelector('h1,h2,h3')||node;if(focus){if(!focus.matches('a,button,input,select,textarea,summary'))focus.tabIndex=-1;focus.focus({preventScroll:true});}}
  scrollTo(node);
 };
 const navigate=(hash,focusTarget=true)=>{if(location.hash!==hash)history.pushState(null,'',hash);reveal(focusTarget);};
 ...
 document.addEventListener('click',event=>{
  ...
  event.preventDefault();navigate(url.hash,!a.closest('.reading-jump-bar'));
 });
