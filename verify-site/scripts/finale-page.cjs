...
const thesis=require('../content/paradigm-finale/paradigm-thesis.cjs');
...
module.exports=({e,origin,date,organization,website,brand,head,footer,write})=>{
 ...
 const desc='The outcome belongs at the beginning. Explore PinnacleAI®’s historical challenge, life-first decision model, source-linked examples, research and licensed scope.';
 ...
 const text=fs.readFileSync(path.join(root,'content/paradigm-finale/PinnacleAI-Paradigm-Shift-Finale.md'),'utf8').replace(/## Production and source register[\s\S]*/, '').replace(/## The principle[\s\S]*?(?=## See what the session)/,thesis.markdown)+'\n\n## Citation\n'+citation+'\n';
 write('evidence/pinnacle-paradigm-shift.txt',text);
 const example=require('../content/paradigm-finale/goal-session-example.cjs');
 const data={title,canonical:url,updated:date,publisher:organization.name,sourceType:'Source-linked company explanation',purpose:source.data.purpose,headline:source.data.headline,historicalChallenge:thesis.data,perspectives:source.data.cards,goalSessionExample:{source:'Owner-supplied planning examples, 22 September 2026; shortened and anonymised',scope:'Proposed activities and criteria, not completed feedback or observed outcomes. No shared record is asserted with the separate report excerpt.',suppliedGoalCount:7,selectedGoals:example.goals,activities:example.activities,sessionMinutes:40,separateFamilyMinutes:5},evidenceDecisions:source.data.evidenceRegister,citation};
 write('evidence/pinnacle-paradigm-shift.json',JSON.stringify(data,null,2));
 return {route,url,text,data,index:'\n## PinnacleAI®: life sets the direction\n- [The complete eighteen-part paradigm story]('+url+'): historical challenge, life-first decision model, anonymised supplied examples and institutional evidence.\n- [Why change the starting point]('+url+'#change-the-starting-point): two visible decision models, with the child’s life governing the PinnacleAI® approach.\n- [Inside a supplied session]('+url+'#documented-example): six activities, three selected goals and forty planned minutes plus five minutes of family handover.\n- [Structured explanation]('+origin+'/evidence/pinnacle-paradigm-shift.json): the same visible claims, planning criteria and evidence boundaries.\n'};
};
