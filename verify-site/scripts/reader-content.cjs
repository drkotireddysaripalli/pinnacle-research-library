...
function organize(body,teaser='',{icon,e,chapterExtras={}}){
 ...
 const bandImage=b=>b.image?`<figure class="reading-band-image"><img src="/images/${b.image}-600.webp" srcset="/images/${b.image}-600.webp 600w, /images/${b.image}-1000.webp 1000w" sizes="(min-width:900px) 520px, calc(100vw - 40px)" width="600" height="400" alt="${e(b.alt)}" loading="lazy" decoding="async"><figcaption>AI-generated conceptual illustration</figcaption></figure>`:b.books?`<div class="reading-band-books" aria-label="Original Pinnacle publication covers">${[['global-research-whitebook','Global research whitebook'],['autism-mothers-handbook-english','Autism mother’s handbook · English'],['autism-mothers-handbook-telugu','Autism mother’s handbook · Telugu']].map(([file,alt])=>`<img src="/images/research/${file}-300.webp" width="300" height="420" alt="${e(alt)}" loading="lazy" decoding="async">`).join('')}</div>`:`<div class="reading-band-mark" aria-hidden="true">${icon(b.icon)}</div>`;
 const intro=`<div class="reader-guide wrap" id="reader-guide"><p class="eyebrow">YOUR QUESTIONS. YOUR PACE.</p><h2>Where would you like to begin?</h2><p>Four sections. Sixteen topics. Open what matters to you.</p></div>`;
 const shortcuts=`<nav class="reading-jump-bar wrap" id="start-here" aria-label="Choose your reading section">${bands.map((b,i)=>`<a href="#reading-${b.id}">${icon(b.icon)}<span>${['Journey','Evidence','Research','Answers'][i]}</span></a>`).join('')}</nav>`;
 const readingSections=bands.map((b,i)=>`<div class="reading-group" id="reading-${b.id}"><div class="reading-band${b.books?' reading-band-research':''}${!b.image&&!b.books?' reading-band-compact':''}">${bandImage(b)}<div class="reading-band-copy"><p class="eyebrow">0${i+1} / ${['THE JOURNEY','THE EVIDENCE','THE KNOWLEDGE','THE ANSWERS'][i]}</p><h2>${e(b.title)}</h2><p>${e(b.text)}</p></div></div><div class="reader-topics" aria-label="${e(b.title)} topics">${chapters.slice(...b.range).join('\n')}</div></div>`).join('');
