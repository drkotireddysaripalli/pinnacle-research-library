...
function enhance(html,url){
 ...
 meta('property','og:title',card.title+' | Pinnacle Verify');meta('property','og:description',card.description);
 meta('name','twitter:image',image);meta('name','twitter:image:alt',card.alt);
 meta('name','twitter:title',card.title+' | Pinnacle Verify');meta('name','twitter:description',card.description);
 // Bibliographic titles stay intact in the visible publication, citation tags and Article graph.
 if(routeOf(url).startsWith('/evidence/publications/'))html=html.replace(/<title>[\s\S]*?<\/title>/,'<title>'+e(card.title+' | Pinnacle Research')+'</title>');
 const description=html.match(/<meta name="description" content="([^"]*)">/)?.[1];
 if(description&&description.length>175)meta('name','description',card.description);
