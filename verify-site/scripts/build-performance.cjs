...
const css=['fonts/fonts.css','styles.css','upgrade.css','evidence-design.css','impact.css','semantic.css','world.css','share.css','reader.css','experience.css','topics.css','citations.css'];
const bundle=fingerprint('site',css.map(p=>fs.readFileSync(path.join(dist,p),'utf8')).join('\n'),'css');
const js={};for(const file of ['share.js','app.js','reader.js','experience.js','hfr-register.js','citations.js'])js[file]=fingerprint(path.basename(file,'.js'),fs.readFileSync(path.join(dist,file),'utf8'),'js');
